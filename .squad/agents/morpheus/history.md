...
- **2026-06-13:** Candidate builds architecture session: designed per-commit deployment lane (`.squad/decisions/inbox/morpheus-candidate-design.md`). URL scheme `/<YYYY-MM-DD>/<N>-<shortsha>/`, 14-day retention, candidates selectable from About tab (CANDIDATE badge). Spec handed off to Tank (CI/CD), Trinity (UI), Link (docs), Smith (tests). Feature merged in PR #1845.
