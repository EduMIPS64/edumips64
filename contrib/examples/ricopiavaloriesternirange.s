;Massimo Trubia
;Given a vector of integer numbers {3.6,40.7,5.67,35.4,6.435,7.6547,46.546,35.454,9.4560,10.456},
;copy all numbers that fall outside the range [6.0;30.0]
;into a second vector, and then compute its sum.
;the separate caches have different associativity
		.data
...
outofrange:	 
		...
		add.d f4,f4,f6			;sum+=vett2[i2]
		daddi r2,r2,8			;i2++
endoutofrange:
incr:		daddi r1,r1,8
		j while_scanvett1
endwhile_scanvett1: syscall 0			;sum is in f4 
...
; {
	...
	; {
		...
		; {
			...
			; i++;
		; }
		; System.out.println(sum); //164  3,40,5,35,46,35
	; }
; }