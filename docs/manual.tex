...
dialog and command line options. The fourth chapter contains some useful
examples.
...
\section{\SC{}}
All the \SC{} (except for \SC{} 0) behave in a similar fashion: they require
that the address of their parameters is saved into register R14.

When a \SC{} is called, it fetches the first parameter from the address
specified in R14; if it needs more than one parameter, they will be fetched from
the the memory areas immediately after the first one. The return value is saved
in R1, and has a different meaning, according to the \SC{} type.

All the \SC{} return $-1$ if there are errors, except for \SC{} 0 that cannot
issue any error.

\subsection{\SC{} 0}
\SC{} 0 doesn't need any argument, and doesn't give back any result value. When
it's called, it stops the execution of the program.
Example:
...
\subsection{\SC{} 1}
\SC{} 1 needs two arguments: the path of the file to open and the opening mode.
