...
spaces and tabs can be used, as the parser ignores multiple spaces.
...
\begin{itemize}
	...
	immediate value sa, and puts the result into rs. Empty bits are padded with
	zeros.
	...
	result into rs. Empty bits are padded with zeros.
	...
	sa, and puts the result into rs. Empty bits are padded with zeros if the
	...
	result into rs. Empty bits are padded with zeros if the leftmost bit of rs
	...
	Does a right shift to of rs, by the amount specified in the immediate value
	sa, and puts the result into rs. Empty bits are padded with zeros.
...
\begin{itemize}
	...
	Loads the constant defined in the immediate value in the upper half of the
	first 32 bits of rt, sign-extending the upper 32 bits of the register.
...
frame, and write there the input.
...
SYSCALL 5 expects a variable number of parameters, the first being a so-called
``format string''. In the format string can be included some placeholders,
