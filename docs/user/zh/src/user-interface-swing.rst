...
单击OK（确定）"按钮将保存选项，而单击Cancel（取消）（或直接关闭窗口）将忽略更改。如果要保存更改，请不要忘记点击确定"。单击"重置为默认值"按钮会在确认后将所有设置恢复为默认值；当模拟器配置错误时，可以用它快速恢复，无需逐项修改。

主设置选项卡允许配置转发、分支延迟槽和多循环模式下的步数。
...
数据前推（Forwarding）
~~~~~~~~~~~~~~~~~~~~~~
EduMIPS64 模拟一个 5 级 MIPS64 流水线（IF、ID、EX、MEM、WB），并可选地
支持数据前推（forwarding），以减少由 Read After Write（RAW）冒险引起的
停顿数量。可以在设置对话框的"主设置"选项卡中启用或禁用前推。

当前推**被禁用**时，源寄存器在 ID 阶段从寄存器堆读取。读取由前两条
指令之一写入的寄存器的指令，必须等待生产者到达 WB 阶段，因此会产生
两次 RAW 停顿。

当前推**被启用**时，模拟器实现了与 Hennessy & Patterson 描述的经典
MIPS 流水线相一致的两条前推路径：

* **EX → EX**：在 EX 阶段末尾产生的 ALU 结果，可以前推到紧接的下一条
  指令的 EX 输入（无停顿）。
* **MEM → EX**：在 MEM 阶段末尾产生的值（通常是 load 的结果，或
  延迟一个周期的 ALU 结果）可以前推到处于 EX 阶段的指令的 EX 输入
  （无停顿）。
...
* **ALU → 分支**：如果分支的源寄存器正由紧邻其前的 ALU 指令产生，
  则在 ID 开始时该值尚不可用，模拟器会插入一次 RAW 停顿。
* **Load → 分支**：如果分支的源寄存器正由紧邻其前的 load 指令产生，
  则该值要到 MEM 阶段末尾才可用，而分支在 ID 开始时就需要它。模拟器
  会插入两次 RAW 停顿：这是 load-use 冒险（1 次停顿）与
  branch-in-ID 冒险（额外 1 次停顿）的叠加。
...
.. note::
   ...
   This section was added together with the English and Italian versions of
   the user manual; the Chinese wording above is a best-effort translation
   and may benefit from review by a native speaker.

分支延迟槽（Branch Delay Slot）
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
EduMIPS64 可以选择性地模拟经典 MIPS 的**分支延迟槽**，正如
Hennessy & Patterson 中所描述的那样。可以在设置对话框的"主设置"选项卡中
启用或禁用延迟槽。

当延迟槽**被禁用**（默认设置）时，紧随分支或跳转之后顺序取出的指令
在 ID 阶段解析跳转时会被*丢弃*：它永远不会进入 EX、MEM 或 WB，
循环视图会在分支之后的槽位显示一个气泡。这是模拟器一贯的行为。

当延迟槽**被启用**时，紧随分支或跳转之后取出的指令（即 *延迟槽*）
**始终被执行**，无论分支是否被采用。分支目标会在延迟槽之后的下一个
周期被取入 IF。这与 MIPS64 指令集的体系结构行为一致。

一些说明：

* 该设置影响所有在 ID 阶段更新程序计数器的控制流指令：无条件分支
  （``B``、``J``、``JAL``）、条件分支（``BEQ``、``BNE``、``BEQZ``、
  ``BNEZ``、``BGEZ``、``BC1T``、``BC1F``）以及基于寄存器的跳转
  （``JR``、``JALR``）。
* 切换延迟槽设置会重置模拟，否则流水线状态将与新的语义不一致。
  在程序运行期间，模拟器会在应用更改前请求确认。
* 假定延迟槽语义编写的程序，通常会在每个分支或跳转之后紧接放置一个
  ``NOP``（或者一个其结果与分支无关的有用指令）。

特殊情况
^^^^^^^^

延迟槽内的若干模式被 MIPS R4000 用户手册（§3.1.2）和 MIPS64
Architecture for Programmers Vol. II 归类为 **UNPREDICTABLE**。
EduMIPS64 为每一种情况选择了确定且文档化的行为，使学习者看到
清晰的诊断信息，而不是被静默破坏的状态：

* **延迟槽中的控制转移指令** —— 将分支或跳转放在另一条分支或
  跳转的延迟槽中，会在运行时抛出
  ``InvalidDelaySlotException`` 异常，消息中会指出问题指令。
  在真实 MIPS 上此情况为 UNPREDICTABLE；教学型模拟器选择直接
  报告错误而非产生任意状态。
* **延迟槽中的 SYSCALL、HALT 或 BREAK** —— 启用延迟槽时，
  延迟槽指令会先执行完毕，然后异常或程序终止才会生效，因此
  syscall 的副作用恰好发生一次，分支目标也不会被取指。禁用
  延迟槽时，延迟槽指令会像普通气泡一样被丢弃。
* **延迟槽中的同步异常** —— 整数溢出、除零等由延迟槽产生的
  EX 阶段异常会在延迟槽推进到 ID 后再被传播，从而使流水线在
  周期结束时处于自洽状态。真实 MIPS 会把 ``EPC`` 设为分支 PC
  并将 ``Cause.BD`` 置 ``1``；EduMIPS64 未建模 CP0，因此直接
  将异常报告在延迟槽指令上。
* **JAL / JALR 的链接寄存器** —— 启用延迟槽时，``R31`` 被设置为
  延迟槽之后那条指令的地址（``JAL_PC + 8``）；禁用延迟槽时，
  ``R31`` 被设置为延迟槽自身的地址（``JAL_PC + 4``）。两种模式
  下都符合 MIPS R4000 的体系结构定义。
* **未采用的分支** —— 延迟槽是体系结构上的顺序后继，
  无论分支是否被采用都会执行。

``EndToEndTests`` 中的端到端测试（搜索 ``testDelaySlot``）
对上述每种情况都进行了固化。
