...
条件分支指令在 **ID** 阶段解析：一旦分支进入译码阶段，就立即读取源寄存器并判断分支条件，这正是 Hennessy 与 Patterson 附录 C 第 C.2 节 "Branch Hazards" 所描述的方式。因此，当分支实际跳转时，已经有两条指令被推测性地取入流水线，必须将它们从流水线中清除；此时分支执行停滞计数器会增加两个单位。

在 ID 阶段解析分支还意味着：若有一条 ALU 指令或 load 指令产生的寄存器恰好被紧随其后的分支指令读取，就会形成一种数据冒险，而此类冒险无法通过转发完全消除。详情及示例可参见 :doc:`pipeline`。
