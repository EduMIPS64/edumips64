...
.. note::
   ...
   一些 MIPS 指令的名称以 ``U`` 结尾（例如 ``ADDU``、``ADDIU``、``DADDU``、
   ``DADDIU``、``DSUBU``、``SUBU``）。尽管带有此后缀，这些指令 **并不会**
   将操作数视为无符号值：结果的符号扩展方式与对应的非 ``U`` 指令完全相同。
   唯一的区别是不会引发整数溢出（Integer Overflow）异常。这是 MIPS 架构中
   一个众所周知的历史性命名误称（misnomer）。
...
* `DMOD rd, rs, rt`
  ...
  计算 64 位寄存器 rs 除以 64 位寄存器 rt 的有符号模（余数），并将结果放入寄存器 rd。
  这是 MIPS64 Release 6 版本。对于将余数存储在 HI 中的传统形式，请使用 DDIV rs, rt 后跟 MFHI。

* `DMUL rd, rs, rt`

  执行 64 位寄存器 rs 和 rt 之间的乘法运算，将运算结果的低阶 64 位双字放入寄存器 rd。
  注意：这是 MIPS64 Release 6 版本。要将结果存储在 LO/HI 寄存器中，请使用 DMULT。
...
* `DSUB rd, rs, rt`.
  ...
  用 64 位寄存器 rs 的值减去 64 位寄存器 rt 的值，将它们视为有符号值，并将结果放入 rd。如果出现溢出，则捕获。

* `DSUBU rd, rs, rt`

  用 64 位寄存器 rs 的值减去 64 位寄存器 rt 的值，并将结果存入 rd。在任何情况下都不会发生整数溢出。
...
* `SUB rd, rs, rt`.
  ...
  用 32 位寄存器 rs 的值减去 32 位寄存器 rt 的值，将它们视为有符号值，并将结果放入 rd。如果出现溢出，则捕获。

* `SUBU rd, rs, rt`

  用 32 位寄存器 rs 的值减去 32 位寄存器 rt 的值，并将结果存入 rd。在任何情况下都不会发生整数溢出。
...
* `ANDI rt, rs, immediate`
  ...
  执行 rs 与立即值之间的位和运算，并将结果存入 rt。16 位立即数采用零扩展，因此接受无符号范围 ``0``–``65535``。
...
* `DADDUI rt, rs, immediate`.
  ...
  .. warning::
     ...
     ``DADDUI`` 不是标准的 MIPS64 指令（它属于 WinMIPS64 指令集，EduMIPS64
     出于向后兼容的目的接受它）。解析器在使用时会发出警告；请改用
     ``DADDIU``。
...
* `ORI rt, rs, immediate`
  ...
  执行 rs 和立即值之间的位操作 OR，将结果放入 rt。16 位立即数采用零扩展，因此接受无符号范围 ``0``–``65535``。
...
* `XORI rt, rs, immediate``
  ...
  在 rs 和立即值之间执行位排他性 OR（XOR），并将结果存入 rt。16 位立即数采用零扩展，因此接受无符号范围 ``0``–``65535``。
...
EduMIPS64 采用 **预测分支不跳转**（branch-not-taken）策略。当遇到条件分支指令时，处理器假设分支不会被执行，并继续取下一条顺序指令。如果分支实际上被执行，则推测取得的指令会从流水线中被清除，执行从分支目标地址处恢复。
...
* `BEQZ rs, offset`
  ...
  .. warning::
     ...
     ``BEQZ`` 不是标准的 MIPS64 指令（它属于 WinMIPS64 指令集，EduMIPS64
     出于向后兼容的目的接受它）。解析器在使用时会发出警告；请改用
     ``BEQ rs, r0, offset``。
...
* `BNEZ rs, offset`.
  ...
  .. warning::
     ...
     ``BNEZ`` 不是标准的 MIPS64 指令（它属于 WinMIPS64 指令集，EduMIPS64
     出于向后兼容的目的接受它）。解析器在使用时会发出警告；请改用
     ``BNE rs, r0, offset``。
...
* `O_TRUNC (0x10)` 在写模式下，打开文件后立即删除文件内容。

必须指定前三种模式之一。第五和第六种模式是排他性的，如果指定 O_TRUNC，则不能指定 O_APPEND（反之亦然）。
...
如果第一个参数为 1 或 2（即 ``stdout`` 或 ``stderr``），模拟器将弹出输入/输出窗口，并在那里写入数据。
...
.. warning::
   ...
   ``HALT`` 不是标准的 MIPS64 指令（它属于 WinMIPS64 指令集，EduMIPS64
   出于向后兼容的目的接受它）。解析器在使用时会发出警告；请改用
   ``SYSCALL 0``。