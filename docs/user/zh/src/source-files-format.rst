...
多个标签可以指向同一个地址。当两个或多个标签在一条指令之前连续放置在不同行上时，这很有用，例如::
...
在这种情况下， `loop_end` 和 `loop_begin` 都将指向同一条指令。每个标签仍然必须具有唯一的名称；
两次使用相同的标签名称将导致解析器错误。
...
整数数据指令（ `.byte` 、 `.word16` 、 `.word32` 、 `.word` 和 `.word64` ）的值除了可以是数字字面量外，
还可以是一个标签。在这种情况下，汇编器会存储该标签所指向的内存地址。该标签既可以引用 `.data` 部分中定义的位置，
也可以引用 `.code` 部分中的某条指令，并且可以在使用它的指令之前或之后定义（因此引用后续代码标签的跳转表也能正常工作）。
解析得到的地址必须能放入该指令的位宽，否则会报告错误；64 位指令（ `.word64` ）总是能容纳一个地址。例如::

    .data
    buffer:   .space   8
    buf_addr: .word64  buffer    ; 存储 "buffer" 的地址
    jump_tbl: .word64  routine   ; 存储代码标签的地址
    .code
...
- *立即值* 立即值可以是一个数字或标签；数字可以以 10 进制、16 进制或 2 进制指定：10 进制数字只需写入数字即可，16 进制数字则需在数字前加上前缀 "0x"，2 进制数字则需在数字前加上前缀 "0b"。立即值的前面可以加上 # 字符。
...
标签偏移运算
^^^^^^^^^^^^
在参数的地址形式中（例如加载或存储指令的偏移量），立即值可以是由内存标签和数字
字面量组成的简单表达式，使用 ``+`` 和 ``-`` 运算符连接。也允许使用开头的 ``+``
或 ``-`` 符号，运算符和操作数之间可以包含空格。每个操作数要么是数字字面量
（10 进制、16 进制或 2 进制），要么是在 ``.data`` 段中定义的内存标签；在对
表达式求值之前，标签会被替换为其地址。

例如，给定数据定义 ``data1: .word 42`` 和 ``data2: .word 43``，以下形式都是
可接受的::

  lw r1, data1+8(r0)       ; 从 data1 向后偏移 8 个字节处加载
  lw r1, data1-8(r0)       ; 从 data1 向前偏移 8 个字节处加载
  lw r1, 0+data1(r0)       ; 等价于 data1(r0)
  lw r1, data2-data1(r0)   ; 两个标签地址之差
  lw r1, data1-8+16(r0)    ; 支持连续使用 + 和 -

包含空操作数的表达式（例如 ``data1+`` 或 ``data1++0``）会被拒绝并视为格式错误，
如果表达式中的某个标签未定义，则会产生通常的“找不到标签”错误。
