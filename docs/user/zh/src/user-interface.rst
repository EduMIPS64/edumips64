...
当模拟器处于 "运行 "或 "多循环 "状态时，"设置... "菜单项不可用。运行 "或 "多循环 "模式时，"设置... "菜单项不可用，因为可能会出现竞赛条件。
...
debug "标志的作用是激活调试模式。在该模式下，会出现一个新的窗口，即调试窗口，显示 EduMIPS64 的内部活动日志。它对最终用户没有用处，仅供 EduMIPS64 开发人员使用。