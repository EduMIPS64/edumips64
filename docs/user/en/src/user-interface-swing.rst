...
source file has been executed and the end has already been reached.
...
* *Change Language* Allows the user to change the language used
  by the user interface. English, Italian and Simplified Chinese are supported.
...
This menu is not available until you have executed a program and the
...
* *Tile* Sorts the visible windows so that no more than three
...
ignored. Don't forget to click "OK" if you want to save your changes. The
