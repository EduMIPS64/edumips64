...
relativo al file da utilizzare (se utilizzate questo listato copia-incollate il codice del listato \ref{lst5.2} prima di questo).}
...
\section{\SC{}}
Tutte le \SC{} (tranne la 0) hanno un comportamento simile: richiedono che
l'indirizzo dei loro parametri sia memorizzato nel registro R14. 

Nel momento in cui viene chiamata una qualunque \SC{} essa preleva il primo
parametro dall'indirizzo specificato in R14; qualora ce ne siano altri, saranno
prelevati dalle aree di memoria immediatamente adiacenti. Il valore di ritorno
\`{e} memorizzato nel registro R1, e varia a seconda della \SC{}. 

Tutte le \SC{} tornano $-1$ in caso di errore, tranne la \SC{} 0, che non può fallire.
\subsection{\SC{} 0}
La \SC{} 0 non richiede alcun parametro in ingresso, e non ha valori di ritorno.
Quando \`e invocata, semplicemente termina l'esecuzione del programma.
Eccone un esempio:
...
\subsection{\SC{} 1}
La \SC{} 1 richiede due parametri: il path del file da aprire e la modalit\'{a} di apertura.\\
Torna il descrittore del file o '-1' se si \'{e} verificato un errore.
Ecco un semplice esempio di programma che apre un file:
\label{lst5.2} \begin{lstlisting} [caption={syscall1}, label={code:syscall1}, style={mips} ] 
      	.data
...
open:
	...
	jal print_string			
\end{lstlisting}
Inizialmente vengono salvati in memoria i dati richiesti dalla \SC{} 1, cioe'
...
r14(l'indirizzo al quale la \SC{} 1 si riferir\'{a}); subito dopo si pu\'{o}
chiamare la \SC{} 1 e salvare il registro r1(che contiene il valore di ritorno
dell \SC{}) in \$s2(in modo da poterlo riutilizzare, per esempio, con le altre
...
\subsection{\SC{} 2}
La \SC{} 2 richiede un solo parametro: il file descriptor del file da chiudere .\\
...
close:	
	...
	jal print_string			
\end{lstlisting}
Inizialmente vengono salvati in memoria i dati richiesti dalla \SC{} 2, cioe'  il file descriptor del file da chiudere(riga 2) e gli si applica un' etichetta
...
(che per il compilatore \'{e} un numero) nel registro r14(l'indirizzo al quale la \SC{} 2 si riferir\'{a}); subito dopo si pu\'{o} chiamare la \SC{} 2.
...
\subsection{\SC{} 3}
La \SC{} 3 richiede tre parametri: il file descriptor da cui leggere('0' per lo standard input), 
...
(che per il compilatore \'{e} un numero) nel registro r14(l'indirizzo al quale la \SC{} 3 si riferir\'{a});dopodich\'{e} salviamo nelle apposite locazioni 
...
Subito dopo si pu\'{o} chiamare la \SC{} 3.A questo  punto viene chiamata la funzione print\_string facendo in modo da passargli l'indirizzo di 'error\_3'
...
La \SC{} 4 richiede tre parametri: il file descriptor su cui scrivere('1' per lo standard output - '2' per lo standard error), 
...
registro r14(l'indirizzo al quale la \SC{} 4 si riferir\'{a});dopodich\'{e}
...
\SC{} 4.A questo  punto viene chiamata la funzione print\_string facendo in
...
\OF{}
\subsection{\SC{} 5}
La \SC{} 5 richiede un numero variabile di parametri (nelle modalità specificate in  \ref{sys5})
...
registro r14 (l'indirizzo al quale la \SC{} 2 si riferir\'{a}); subito dopo si
pu\'{o} chiamare la \SC{} 5 che stampa nello standard output (una fiestra
