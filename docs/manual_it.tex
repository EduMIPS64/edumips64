...
open:
	...
	jal print_string			
\end{lstlisting}
Inizialmente vengono salvati in memoria i dati richiesti dalla syscall 1, cioe' la stringa contenente il path del file da aprire(riga 2), 
e (nella locazione di memoria immediatamente successiva) un intero che ne definisce la modalita' di apertura(riga 3).Si applica un' etichetta
...
di successo ed errore da passare alla funzione 'print_string'(righe 5, 7).Passando alla sezione .text, inizialmente si salva l'indirizzo di params_sys1
(che per il compilatore \'{e} un numero) nel registro r14(l'indirizzo al quale la syscall 1 si riferir\'{a}); subito dopo si pu\'{o} chiamare la syscall 1 e 
salvare il registro r1(che contiene il valore di ritorno dell syscall) in $s2(in modo da poterlo riutilizzare, per esempio, con le altre syscall).A questo 
punto viene chiamata la funzione print_string facendo in modo da passargli l'indirizzo di 'error_op' nel caso in cui r1 sia uguale a -1(righe 18-19).
